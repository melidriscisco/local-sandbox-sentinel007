...
from .state import Message, Type as MsgType
import faiss  # make faiss available


def get_last_message(messages, msg_type=MsgType.ai) -> str:
    for m in reversed(messages):
        if isinstance(m, Message):
            if m.type == msg_type:
                return m.content
        if isinstance(m, dict):
            if m["type"] == str(msg_type).split(".")[-1]:
                return m["content"]
    return ""


def get_all_messages(messages) -> str:
    all_messages = []
    for m in messages:
        if isinstance(m, Message):
