...
from sentinel007 import intention_analyzer, state, jailbreak_judge, jailbreak_prompt_analyzer
...
from sentinel007.state import SendGridState, IntentionAnalyzerState
...
def prepare_output(state: state.OverallState, config:RunnableConfig) -> state.OverallState:
    state.messages = copy.deepcopy(
        state.intention_analyzer_state.output.messages if (state.intention_analyzer_state
            and state.intention_analyzer_state.output
            and state.intention_analyzer_state.output.messages
        ) else []
    )
...
def build_graph() -> CompiledStateGraph:
    ...
    sg.add_edge(acp_prompt_analyzer.get_name(), "prepare_sendgrid_input")
    sg.add_edge(acp_judge.get_name(), "prepare_sendgrid_input")
